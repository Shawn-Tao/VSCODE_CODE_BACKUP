...
int main(int argc, char **argv) {
  ...
  // load image
  assert(argv[1] != nullptr && argv[2] != nullptr);
  
  cv::Mat first_image = cv::imread(argv[1], 0);
  cv::Mat second_image = cv::imread(argv[2], 0);

  // cv::Mat first_image_1 = cv::imread(argv[1], 0);
  // cv::Mat second_image_1 = cv::imread(argv[2], 0);
  // cv::Mat first_image = first_image_1.colRange(300,first_image_1.cols-300).rowRange(300,first_image_1.rows-1);
  // cv::Mat second_image = second_image_1.colRange(300,second_image_1.cols-300).rowRange(300,second_image_1.rows-1);
  
  //assert函数如果值为假,则停止程序
  assert(first_image.data != nullptr && second_image.data != nullptr);

  // detect FAST keypoints1 using threshold=40 ,并计算ORB特征,均以vector保存
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
  ...
  for (auto &kp: keypoints) {
    ...
    // compute the angle of this point提取BRIEF描述子的同时计算了关键点的方向,构成256维度 0 1构成的描述子
    DescType desc(8, 0);
    ...
    for (int i = 0; i < 8; i++) {
      ...
      for (int k = 0; k < 32; k++) {
        ...
        cv::Point2f qq = cv::Point2f(cos_theta * q.x - sin_theta * q.y, sin_theta * q.x + cos_theta * q.y)
                         + kp.pt;
        //创建一个二进制1,然后左移k位(比如k=2的话,就变成0100),然后按位 "或运算" 
